...
app.get("/api/kantor-data", (req, res) => {
    db_1.default.query("SELECT * FROM kantory", (err, result) => {
        if (err) {
...
app.post("/api/add-new-currency-exchange", (req, res) => {
    ...
    const kantorDataFields = Object.keys(requestData).filter((field) => field in requestData && requestData[field] !== '');
    ...
    db_1.default.query(sqlQuery, values, (err, result) => {
        if (err) {
